...
import SLArpaas_test11_RegisterFile
from ctypes import *
import array
import numpy as np
...
def REG_ManualTrigger_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_ManualTrigger, handle)
    return err, data

def REG_ManualTrigger_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_ManualTrigger, handle)
    return err

def REG_EnExternalTrigger_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_EnExternalTrigger, handle)
    return err, data

def REG_EnExternalTrigger_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_EnExternalTrigger, handle)
    return err

def REG_Threshold0_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_Threshold0, handle)
    return err, data

def REG_Threshold0_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_Threshold0, handle)
    return err

def REG_Polarity0_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_Polarity0, handle)
    return err, data

def REG_Polarity0_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_Polarity0, handle)
    return err

def REG_EnTrigger0_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_EnTrigger0, handle)
    return err, data

def REG_EnTrigger0_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_EnTrigger0, handle)
    return err

def REG_TimingReset_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_TimingReset, handle)
    return err, data

def REG_TimingReset_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_TimingReset, handle)
    return err

def REG_CounterEnable_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_CounterEnable, handle)
    return err, data

def REG_CounterEnable_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_CounterEnable, handle)
    return err

def REG_CounterReset_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_CounterReset, handle)
    return err, data

def REG_CounterReset_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_CounterReset, handle)
    return err

def REG_Counts_GET(handle):
    [err, data] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_Counts, handle)
    return err, data

def REG_Counts_SET(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_Counts, handle)
    return err



def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(1024, SLArpaas_test11_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def LISTMODULE_Digitizer_0_RESET(handle):
    err = __abstracted_reg_write(2, SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    return err

def LISTMODULE_Digitizer_0_START(handle):
    err = __abstracted_reg_write(2, SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    return True

def LISTMODULE_Digitizer_0_START(nchannels, handle):
    err = __abstracted_reg_write(2+(nchannels<<8), SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(0+(nchannels<<8), SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1+(nchannels<<8), SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_CONFIG, handle)
    if (err != 0):
       return False
    return True

def LISTMODULE_Digitizer_0_SetLen(data, handle):
    err = __abstracted_reg_write(data, SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_ACQ_LEN, handle)
    return err

def LISTMODULE_Digitizer_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_STATUS, handle)
    status = status & 0xf
    return err, status

def LISTMODULE_Digitizer_0_GET_DATA(Data_Number, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_fifo_read(Data_Number, SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_FIFOADDRESS, SLArpaas_test11_RegisterFile.SCI_REG_Digitizer_0_STATUS, 1, timeout_ms, handle)
    return err, data, read_data, valid_data
